...
async function main() {
  ...
  // 5. Create Mock Credentials (So UI isn't completely empty initially)
  const code = crypto.randomBytes(4).toString('hex').toUpperCase();
  const issueDate = new Date();
  
  // Create a real hash for the mock credential so verification passes!
  const SECRET_KEY = process.env.CREDENTIAL_SECRET_KEY || 'dezai-secret-key';
  const dataString = `${student.name}|Hackathon Winner|${issueDate.toISOString()}`;
  const validHash = crypto.createHmac('sha256', SECRET_KEY).update(dataString).digest('hex');
  ...
  await prisma.credential.upsert({
    ...
    create: {
      ...
      programId: program.id,
      institutionId: institution.id,
      issuedById: faculty.id,
      tier: 'CITADEL',
      verificationCode: code,
      verificationUrl: `http://localhost:3000/verify/${code}`,
      verificationStatus: VerifyStatus.ACTIVE,
      approvalStatus: ApprovalStatus.APPROVED,
      metadata: JSON.stringify({ type: 'MERIT', awardTitle: 'Hackathon Winner' }),
      hashSignature: validHash,
      issuedAt: issueDate,
    },
  });
  ...
  const validHashPending = crypto.createHmac('sha256', SECRET_KEY).update(`${student.name}|Outstanding Performance|${issueDate.toISOString()}`).digest('hex');
  await prisma.credential.create({
    data: {
      id: 'cred-mock-pending',
      user: { connect: { id: student.id } },
      program: { connect: { id: program.id } },
      institution: { connect: { id: institution.id } },
      issuer: { connect: { id: faculty.id } },
      tier: CredentialTier.FORGE,
      verificationCode: codePending,
      verificationUrl: `http://localhost:3000/verify/${codePending}`,
      verificationStatus: VerifyStatus.ACTIVE,
      approvalStatus: ApprovalStatus.PENDING,
      metadata: JSON.stringify({ type: 'MERIT', awardTitle: 'Outstanding Performance' }),
      hashSignature: validHashPending,
      issuedAt: issueDate,
    }
  });
  ...
  const validHashSusp = crypto.createHmac('sha256', SECRET_KEY).update(`${student.name}|Best Project|${issueDate.toISOString()}`).digest('hex');
  await prisma.credential.create({
    data: {
      id: 'cred-mock-susp',
      user: { connect: { id: student.id } },
      ...
      institution: { connect: { id: institution.id } },
      issuer: { connect: { id: faculty.id } },
      tier: CredentialTier.ARENA,
      verificationCode: codeSuspended,
      verificationUrl: `http://localhost:3000/verify/${codeSuspended}`,
      verificationStatus: VerifyStatus.SUSPENDED,
      approvalStatus: ApprovalStatus.APPROVED,
      metadata: JSON.stringify({ type: 'MERIT', awardTitle: 'Best Project' }),
      hashSignature: validHashSusp,
      issuedAt: issueDate,
    }
  });
